...
def cache_path(config: LeanConfig) -> Path:
    """Return the absolute shared cache directory selected by ``config``.

    This is ``config.cache_dir`` when set, otherwise the platform's AI Functions
    cache. The directory is not created.
    """
    return Path(config.cache_dir or platformdirs.user_cache_dir("ai_functions")).expanduser().resolve()

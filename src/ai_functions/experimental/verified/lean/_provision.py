...
from .execution import Deadline, file_lock, run_command
from .toolchain import LeanConfig, ResolvedToolchain, cache_path
...
def _download(url: str, destination: Path, *, timeout: float) -> None:
    ...
    deadline = Deadline(timeout, "Lean installer download exceeded its deadline")
    try:
...
# `lean --version` output by binary identity and environment: one process per
# installation per Python process, not one per project.
_VERSIONS: dict[tuple[object, ...], str] = {}
...
def _resolved(bin_dir: Path, pin: str, environment: dict[str, str], timeout: float) -> ResolvedToolchain:
    ...
    command_environment = tools.environment()
    info = lean.stat()
    key = (
        str(lean),
        info.st_dev,
        ...
        info.st_size,
        info.st_mtime_ns,
        tuple(sorted(command_environment.items())),
    )
    version = _VERSIONS.get(key)
    if version is None:
        version = run_command([str(lean), "--version"], environment=command_environment, timeout=timeout).stdout.strip()
...
def resolve_toolchain(
    ...
    *,
    ...
    remaining = Deadline(timeout, "Toolchain setup exceeded its deadline").remaining
    ...
    with file_lock(destination.parent / ".locks" / f"{destination.name}.lock", timeout=remaining()):
